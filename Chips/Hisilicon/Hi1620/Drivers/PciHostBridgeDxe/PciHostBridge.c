  ...
  )
{
  ...
  for (Loop1 = 0; Loop1 < PCIE_MAX_HOSTBRIDGE; Loop1++) {
    // if 1P, only create for HB0.
    if ((!OemIsMpBoot()) && (Loop1 == 0x01)) {
      break;
    }
    ...
    HostBridge->RootBridgeNumber = RootBridgeNumber[Loop1];  // How many PCIE ports?
    ...
    for (Loop2 = 0; Loop2 < HostBridge->RootBridgeNumber; Loop2++) {
      ...
      Status = gDS->AddMemorySpace (
             ...
             mResAppeture[Loop1][Loop2].Mem32Base,
             mResAppeture[Loop1][Loop2].Mem32Limit - mResAppeture[Loop1][Loop2].Mem32Base + 1,
             0
      );

      if (EFI_ERROR (Status)) {
        DEBUG((EFI_D_ERROR,"PCIE AddMemory 32-Bit Space Error\n"));
      }

      // PCI 64-bit Memory Space
      Status = gDS->AddMemorySpace (
             EfiGcdMemoryTypeMemoryMappedIo,
             mResAppeture[Loop1][Loop2].Mem64Base,
             mResAppeture[Loop1][Loop2].Mem64Limit - mResAppeture[Loop1][Loop2].Mem64Base + 1,
             0
      );

      if (EFI_ERROR (Status)) {
        DEBUG((EFI_D_ERROR,"PCIE AddMemory 64-Bit Space Error\n"));
      }
...
)
{
  ...
  if ((Index == TypeMem32) || (Index == TypePMem32)) {
    BaseAddress = (RootBridgeInstance->Mem32Base + *AllocatedLenMem +
                       RootBridgeInstance->ResAllocNode[Index].Alignment)
                        & ~(RootBridgeInstance->ResAllocNode[Index].Alignment);

    if ((BaseAddress + AddrLen - 1) > RootBridgeInstance->Mem32Limit) {
          ReturnStatus = EFI_OUT_OF_RESOURCES;
          RootBridgeInstance->ResAllocNode[Index].Length = 0;
          return ReturnStatus;
    }
  }else if ((Index == TypeMem64) || (Index == TypePMem64)) {
    BaseAddress = (RootBridgeInstance->Mem64Base + *AllocatedLenMem +
                       RootBridgeInstance->ResAllocNode[Index].Alignment)
                        & ~(RootBridgeInstance->ResAllocNode[Index].Alignment);

    if ((BaseAddress + AddrLen - 1) > RootBridgeInstance->Mem64Limit) {
          ReturnStatus = EFI_OUT_OF_RESOURCES;
          RootBridgeInstance->ResAllocNode[Index].Length = 0;
          return ReturnStatus;
    }
  ...
  while (List != &HostBridgeInstance->Head) {
    ...
    for (Index = TypeIo; Index < TypeBus; Index++) {
      if (RootBridgeInstance->ResAllocNode[Index].Status != ResNone) {
        if(Index == TypeIo) {
          PCIE_DEBUG("SUPPOER IO RESOURCES ON THIS PLATFORM\n");
          UINTN BaseAddr = 0xE7FF0000;

          RootBridgeInstance->ResAllocNode[Index].Base = BaseAddr;
          RootBridgeInstance->ResAllocNode[Index].Status = ResAllocated;
  ...
  for (Index = 0; Index < TypeBus; Index ++)
  {
    if (RootBridgeInstance->ResAllocNode[Index].Status != ResNone) {
      ...
      case TypeMem32:
        ...
        Ptr->AddrRangeMin = (RootBridgeInstance->ResAllocNode[Index].Base - RootBridgeInstance->Mem32Base) +
                             (RootBridgeInstance->Mem32Base & 0xFFFFFFFF);
        Ptr->AddrRangeMax = 0;
        Ptr->AddrTranslationOffset = (ResStatus == ResAllocated) ? EFI_RESOURCE_SATISFIED : EFI_RESOURCE_LESS;
        Ptr->AddrLen = RootBridgeInstance->ResAllocNode[Index].Length;
        break;
      ...
      case TypeMem64:
        ...
        /* This is PCIE Device Bus which start address is the low 32bit of mem base*/
        Ptr->AddrRangeMin = (RootBridgeInstance->ResAllocNode[Index].Base - RootBridgeInstance->Mem64Base) +
                             (RootBridgeInstance->Mem64Base & 0xFFFFFFFFFFFFFFFF);
        Ptr->AddrRangeMax = 0;
        Ptr->AddrTranslationOffset = (ResStatus == ResAllocated) ? EFI_RESOURCE_SATISFIED : EFI_RESOURCE_LESS;
        Ptr->AddrLen = RootBridgeInstance->ResAllocNode[Index].Length;
        break;
