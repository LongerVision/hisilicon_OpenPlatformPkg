  ...
  )
{
  ...
  PrivateData->Mem32Base = ResAppeture->Mem32Base;
  PrivateData->Mem32Limit = ResAppeture->Mem32Limit;
  PrivateData->IoBase  = ResAppeture->IoBase;
  PrivateData->IoLimit  = ResAppeture->IoLimit;
  PrivateData->Mem64Base = ResAppeture->Mem64Base;
  PrivateData->Mem64Limit = ResAppeture->Mem64Limit;
  //
  ...
  )
{
  ...
  if (OperationType == IoOperation) {
    ...
    if (Address < SIZE_4GB) {
      ...
      Limit = PrivateData->Mem32Limit;
    }else{
      Base = PrivateData->Mem64Base;
      Limit = PrivateData->Mem64Limit;
    }
  } else {
    ...
    if (PrivateData->ResAllocNode[Index].Status == ResAllocated) {
      switch (Index) {
        ...
        case TypeBus:
          ...
          break;
        default:
	    /* PCIE Device bar address should be base on Base */
        Configuration.SpaceDesp[Index].AddrRangeMin = PrivateData->ResAllocNode[Index].Base;
      }
